...
void saveSettings(bool wipeAllProfileData)
{
	...
	{ // If wipe all tips is not set
		if (systemSettings.settings.currentProfile != profile_None)
		{									// If current tip is initialized
...
void restoreSettings()
{
	...
	{ // If flash not initialized (Erased flash is always read "1")
		resetSystemSettings();
...
void loadProfile(uint8_t profile)
{
	...
	{									    // If stored temps are in different units
...
void settingsChkErr(void)
{
	...
	{ // If current tip type is valid
		...
		if (systemSettings.ProfileChecksum == ChecksumProfile(&systemSettings.Profile))
		{							      // If current profile checksum is correct
...
void ErrCountDown(uint8_t Start, uint8_t xpos, uint8_t ypos)
{
	...
	while (oled.status != oled_idle)
		; // Wait for the srceen to be idle (few mS at most). Hanging here will cause a watchdog reset.
