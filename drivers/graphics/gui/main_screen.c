...
static void setTip(uint8_t *val)
{
	...
	{ // Tip temp uses huge font that partially overlaps other widgets
...
int main_screenProcessInput(screen_t *scr, RE_Rotation_t input, RE_State_t *state)
{
	...
	if (mainScr.update)
	{ // This was set on a previous pass. We reset the flag now.
		mainScr.update = 0;
	}
	...
	case main_irontemp:
		...
		{					       // When the screen goes to disable state
...
void main_screen_draw(screen_t *scr)
{
	...
	if (mainScr.currentMode != main_disabled && (HAL_GetTick() - plotTime) > 99)
	{ // Only store values if running
		plotUpdate = 1;
		scr->refresh = screenRefresh_eraseNow;
	...
	if (scr_refresh)
	{
		...
		if (mainScr.currentMode == main_disabled)
		{
			...
			if (mainScr.ironStatus == status_error)
			{
				...
				if (Iron.Error.Flags == 0x81)
				{ // 0x81 = Only "No iron detected". Don't show error just for it
					u8g2_SetFont(&u8g2, u8g2_font_mainBig);
	...
	if (mainScr.ironStatus == status_running)
	{
		if (scr_refresh || (HAL_GetTick() - barTime) > 9)
		{ // Update every 10mS or if screen was erased
			if (scr_refresh < screenRefresh_eraseNow)
			{					 // If screen not erased
		...
		{ //Update every 100mS or if screen is erased
			plotUpdate = 0;
			...
			{ // graphing magnified
				...
				for (uint8_t x = 0; x < 100; x++)
				{
					...
					if (plotV < t - 20)
						...
						plotV = (plotV - t + 20);		  // relative to t, +-20C
			...
			{ // graphing full range
				...
				for (uint8_t x = 0; x < 100; x++)
				{
					...
					if (plotV < 180)
						...
						plotV = (plotV - 180) >> 3;		  // divide by 8, (500-180)/8=40
					u8g2_DrawVLine(&u8g2, x + 13, 56 - plotV, plotV); // data points
				}
